...
$(call anrem-export-module, calc)

######################################################################## UNRESOLVED
CALC_obj_$(CURRENT) := test_src/calc/calc.o
########################################################################

# for example the required files are defined as non-local
CALC_deps = $(addprefix $(CURRENT)/,hello.c)
CALC_obj = $(CALC_deps:%.c=%.o)

$(call anrem-target, $(CALC_obj)): $(CALC_deps)
	$(CC) -c -o $@ $^

$(call anrem-target, calc_clean):
	rm -f $(path)/*.o

$(call anrem-clean, calc_clean)
